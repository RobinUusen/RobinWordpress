...
class Falang_Public extends Falang_Rewrite{
	...
	public function load() {
		...
		if ($this->current_language = $this->get_current_language()) {
			...
			add_filter('the_content', array($this, 'translate_post_content'), 8);//change 9 to 8 for display post type plugin working
...
//			add_filter('get_pages', array($this, 'translate_the_pages'), 10, 2);
	...
	public function catch_translation($query_vars) {
		...
		$falang_taxo = new \Falang\Core\Taxonomy(null,$this->model);
...
//		}
	...
	public function translate_post_content($content) {
		global $post;
		$falang_post = new \Falang\Core\Post($post->ID);
		//Divi edition page don't translate content
				...
				foreach ($meta_vals as $key => $val) {
					...
					if (in_array($key, $falang_post->get_post_type_metakeys($object->post_type))) {
						...
						if (apply_filters('falang_postmeta_override', $meta_val, $key, $object_id)) {
							...
							$meta_vals[$key] = is_array($val) ? array($meta_val) : $meta_val;
		...
		if ($menu_item->type == 'post_type') {
			...
			if ($falang_post->is_post_type_translatable($menu_item->object)) {
				...
				if (empty($menu_item->post_title)) {
					...
					$menu_item->title = $falang_post->translate_post_field($original_post, 'post_title', null, $menu_item->title);
...
//				$language_index++;
	...
	public function translate_nav_menu_item($menu_item, $fill_default_title = false) {
		...
		if (!$this->is_default() && $falang_post->is_post_type_translatable('nav_menu_item')) {
			...
			$menu_item->post_title = $falang_post->translate_post_field($menu_item, 'post_title', null, ($fill_default_title ? $menu_item->title : ''));
			$menu_item->description = $falang_post->translate_post_field($menu_item, 'post_content', null, $menu_item->description);
			$menu_item->attr_title = $falang_post->translate_post_field($menu_item, 'post_excerpt', null, $menu_item->attr_title);
...
//					$language->name,
//					($this->is_default($language) ? ' current' : ''),
//					$this->get_translated_url($language),
...
//				);
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/falang-public.js', array( 'jquery' ), $this->version, false );
...
//	}
	 ...
	 * Shortcode [falang lang="en_EN"] [/falang]
	 *
	 * @since 1.3.1
	 */
    ...
    public function translate_attachment_caption($caption, $post_id) {
        ...
        if ($post && !$this->is_default() && $falang_post->is_post_type_translatable($post->post_type)) {
            ...
            $caption = $falang_post->translate_post_field($post, 'post_excerpt', $this->current_language, $caption);
