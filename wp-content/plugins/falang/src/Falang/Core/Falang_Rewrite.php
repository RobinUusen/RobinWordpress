...
class Falang_Rewrite extends  Falang_Core {
	 ...
	 * @from 2.0
	 * @since 1.3.3 fix has_archive notice undefined.
	 */
	public function register_post_type_args($args, $post_type) {
		$falang_post= new Post();

		//1.3.3 change the get_option and add is_admin see -> WP_Post_Type::set_props()
		if ($falang_post->is_post_type_translatable($post_type) && isset($args['rewrite']) && $args['rewrite'] !== false && (is_admin() || get_option('permalink_structure'))) {
	...
	public function register_taxonomy_args($args, $taxonomy) {
		...
		$falang_taxonomy = new Taxonomy(null,$this->model);
	...
	public function registered_taxonomy($taxonomy, $object_type, $taxonomy_obj) {
		...
		$falang_taxo = new Taxonomy(null,$this->model);
			...
			foreach ($page_query->posts as $page) {
				...
				if ($slugs) {
					...
					if (apply_filters('falang_page_use_pagination', false)) {
						...
						$duplicate_rules[$pageregex] =  $wp_rewrite->index . '?falang_page=$matches[1]&page=$matches[2]';
