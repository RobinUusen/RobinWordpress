...
//		add_action('init', array($this, 'register_extra_post'), 20);
    ...
    public function enqueue_scripts()
    {
        ...
        wp_enqueue_script($this->plugin_name, plugin_dir_url(__FILE__) . 'js/falang-admin.js', array('jquery'), $this->version, false);
    ...
    public function ajax_enqueue_scripts()
    {
        ...
        wp_register_script('falang-ajax', plugin_dir_url(__FILE__) . 'js/ajax.js', array('jquery'), false, true);
    ...
    public function languages_page()
    {
        ...
        switch ($this->active_tab) {
            ...
            case 'translation':
                ...
                //TODO probably put a configuration in settings to have specifc menu and remove it here
                //TODO filter by post type and other filter in the same get_posts
                //if post_type filter is used exclude all post_type without the filter TODO put this in other place
                ...
                if ((isset($_REQUEST['pt-filter']) && $_REQUEST['pt-filter'])) {
                    ...
                    $translable_post_list = get_posts(array(
//						'post_type'   => $this->model->get_translated_post_types( true, $exluded_post_type ),
            ...
            case 'options' :
                ...
                if (isset($_POST['s']) && $_POST['s']) {
                    $translable_options_list = $this->get_options_to_translate($_POST['s']);
                } else {
...
//		}
    ...
    public function handle_strings_actions($action)
    {
        ...
        // save string (normally done directoy by ajax
    ...
    public function save_settings()
    {
        if (current_user_can('manage_options')) {
            ...
            if (isset($_POST['falang_settings_option']) && wp_verify_nonce($_POST['falang_settings_option'], 'falang_action')) {
                ...
                $falang_tax = new \Falang\Core\Taxonomy(null, $this->model);
    ...
    public function save_settings_options()
    {
        if (current_user_can('manage_options')) {
            ...
            if (isset($_POST['falang_post_option'], $_POST['post_type']) && wp_verify_nonce($_POST['falang_post_option'], 'falang_action')) {
                ...
                $Taxonomy = new \Falang\Core\Taxonomy(null, $this->model);
...
//		$target_language_locale = sanitize_html_class($_GET['language']);//local in the url
    ...
    public function display_edit_menu_page()
    {
        ...
        $aData = array(
            'form_action' => 'admin.php?page=falang-menus',
            'cancel_action' => 'admin.php?page=falang-menus',
...
//			if (!$this->is_default()) {
                ...
                $permalink[1] = $name ? $name : $falang_post->translate_post_field($post, 'post_name');
